...
try:
    ...
    causal_conv1d_fn, causal_conv1d_update = None # type: ignore[attr-defined]
...
class Mamba(nn.Module):
    def __init__(
        ...
        expand=2,
        dt_rank="auto",
        dt_min=0.001, #时间步的范围 delta越小说明停滞越关注当前信息，反之越大说明快速掠过
        dt_max=0.1,
        dt_init="random",
        dt_scale=1.0,
        dt_init_floor=1e-4,
        ...
        self.d_conv = d_conv
        self.expand = expand
        self.d_inner = int(self.expand * self.d_model) #扩展通道维度为倍数
        ...
        self.init_layer_scale = init_layer_scale #初始化添加一个layer scale参数
        if init_layer_scale is not None: #对于音频视频分支分别应用layer scale
        ...
        self.a_in_proj = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)  #将输入的通道扩展成俩份
        self.v_in_proj = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)


        self.a_conv1d = nn.Conv1d(  #音频视频分支各自的卷积
            ...
            groups=self.d_inner,
            padding=d_conv - 1,
            **factory_kwargs, # type: ignore[attr-defined]
        )
        ...
        self.a_x_proj = nn.Linear( #生成数据依赖的bc delta
            self.d_inner, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs # type: ignore[attr-defined]
        )
        self.a_dt_proj = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs) #低秩 delta 表示恢复 full size 的 delta 向量 # type: ignore[attr-defined]

        self.v_x_proj = nn.Linear(
            self.d_inner, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs# type: ignore[attr-defined]
        )
        self.v_dt_proj = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs)# type: ignore[attr-defined]


        # Initialize special dt projection to preserve variance at initialization
        dt_init_std = self.dt_rank**-0.5 * dt_scale #初始dt的权重# type: ignore[attr-defined]
        if dt_init == "constant": #每个dt都差不多大稳健
            nn.init.constant_(self.a_dt_proj.weight, dt_init_std)
            nn.init.constant_(self.v_dt_proj.weight, dt_init_std)
        elif dt_init == "random": #每个dt都随机初始化可能更强大
        ...
        self.a_dt_proj.bias._no_reinit = True # type: ignore[attr-defined]
        ...
        # Our initialization would set all Linear.bias to zero, need to mark this one as _no_reinit
        self.v_dt_proj.bias._no_reinit = True # type: ignore[attr-defined]
        ...
        self.a_D._no_weight_decay = True # type: ignore[attr-defined]
        self.v_D = nn.Parameter(torch.ones(self.d_inner, device=device))  # Keep in fp32
        self.v_D._no_weight_decay = True # type: ignore[attr-defined]
        ...
        if bimamba_type == "v1":
        ...
        elif bimamba_type == "v2": #俩个模态双向mamba
            A_b = repeat(
                torch.arange(1, self.d_state + 1, dtype=torch.float32, device=device),
                "n -> d n",
                d=self.d_inner,
            ...
            self.a_dt_proj_b = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs) # type: ignore[attr-defined]

            self.v_x_proj_b = nn.Linear(
                self.d_inner, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs # type: ignore[attr-defined]
            )
            self.v_dt_proj_b = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs) # type: ignore[attr-defined]

            self.a_D_b = nn.Parameter(torch.ones(self.d_inner, device=device))  # Keep in fp32
            self.a_D_b._no_weight_decay = True # type: ignore[attr-defined]
            self.v_D_b = nn.Parameter(torch.ones(self.d_inner, device=device))  # Keep in fp32
            self.v_D_b._no_weight_decay = True # type: ignore[attr-defined]
    ...
    def forward(self, a_hidden_states, v_hidden_states, a_inference_params=None, v_inference_params=None):
        ...
        A = -torch.exp(self.A_log.float())  # (d_inner, d_state) 俩个模态共享
        ...
        if self.use_fast_path and a_inference_params is None and v_inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v1":
                A_b = -torch.exp(self.A_b_log.float()) #俩个模态共享a反矩阵
                ...
                A_b = -torch.exp(self.A_b_log.float()) #俩个模态共享a反
                ...
                if not self.if_devide_out:
                    a_out = F.linear(rearrange(a_out + a_out_b.flip([-1]), "b d l -> b l d"), self.a_out_proj.weight, self.a_out_proj.bias) # type: ignore[attr-defined]
                    v_out = F.linear(rearrange(v_out + v_out_b.flip([-1]), "b d l -> b l d"), self.v_out_proj.weight, self.v_out_proj.bias) # type: ignore[attr-defined]
                else:
                    a_out = F.linear(rearrange(0.5*a_out + 0.5*a_out_b.flip([-1]), "b d l -> b l d"), self.a_out_proj.weight, self.a_out_proj.bias) # type: ignore[attr-defined]
                    v_out = F.linear(rearrange(0.5*v_out + 0.5*v_out_b.flip([-1]), "b d l -> b l d"), self.v_out_proj.weight, self.v_out_proj.bias) # type: ignore[attr-defined]
            else:
            ...
            a_dt, a_B, a_C = torch.split(a_x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1) #生成dt b c # type: ignore[attr-defined]
            ...
            v_dt, v_B, v_C = torch.split(v_x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1) # type: ignore[attr-defined]
    ...
    def step(self, a_hidden_states, a_conv_state, a_ssm_state, v_hidden_states, v_conv_state, v_ssm_state):
        ...
        a_dt, a_B, a_C = torch.split(a_x_db, [self.dt_rank, self.d_state, self.d_state], dim=-1) # type: ignore[attr-defined]
        # Don't add dt_bias here
        a_dt = F.linear(a_dt, self.a_dt_proj.weight)  # (B d_inner)

        v_x_db = self.v_x_proj(v_x)  # (B dt_rank+2*d_state)
        v_dt, v_B, v_C = torch.split(v_x_db, [self.dt_rank, self.d_state, self.d_state], dim=-1) # type: ignore[attr-defined]
        # Don't add dt_bias here
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if not self.fused_add_norm:
            ...
            fused_add_norm_fn = rms_norm_fn if isinstance(self.norm, RMSNorm) else layer_norm_fn # type: ignore[attr-defined]
            hidden_states, residual = fused_add_norm_fn( # type: ignore[attr-defined]
                hidden_states,
                self.norm.weight,
                self.norm.bias,
                residual=residual,
